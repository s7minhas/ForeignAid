...
simPlots = lapply(1:length(stratMuIntMods), function(i){
  ...
  scen = scen[,-1]
  scen = cbind( scen, scen[,2]*scen[,3] )
  colnames(scen) = names(coef(mod)[c(1:7,length(coef(mod)))])
  scen = data.matrix(scen)
  pred = scen %*% coef(mod)[c(1:7,length(coef(mod)))]
  draws = mvrnorm(10000, 
  	coef(mod)[c(1:7,length(coef(mod)))], 
  	vcov(mod)[c(1:7,length(coef(mod))),c(1:7,length(coef(mod)))]
  	)
  sysUncert = scen %*% t(draws)
  sysInts95 = t(apply(sysUncert, 1, function(x){
    quantile(x, c(0.025, 0.975), na.rm=TRUE) }))
