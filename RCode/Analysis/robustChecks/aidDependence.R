...
for(i in 1:length(dvs)){
  ...
  simResults = lapply(1:length(stratMuIntMods), function(j){
    mod = stratMuIntMods[[j]] ; var = names(fixef(mod))[3]
    disVar = names(fixef(mod))[4]
...
ggsave(simPlots[[1]], 
  file=paste0(pathGraphics, '/simComboPlot_lag_hAid_lagDV.pdf'), 
  width=7, height=4)
ggsave(simPlots[[2]], 
  file=paste0(pathGraphics, '/simComboPlot_lag_dAid_lagDV.pdf'), 
  width=7, height=4)
ggsave(simPlots[[3]], 
  file=paste0(pathGraphics, '/simComboPlot_lag_cAid_lagDV.pdf'), 
  width=7, height=4)
################################################################