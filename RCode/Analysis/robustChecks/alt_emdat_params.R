...
simPlots = lapply(1:length(stratMuIntMods), function(i){
  mod = stratMuIntMods[[i]][[1]]
  modTitle = dvNames[i] ; var = 'Lno_killed'
  ...
  # Plot rel at various cuts of disasters
  disRange=with(data=regData, seq(
    min(Lno_killed), noDisast, 2) )    
  ggDataSmall = ggData[which(ggData$Lno_killed %in% disRange),]
  actData = regData[,c('LstratMu', 'Lno_killed')]
  actData = actData[actData$Lno_killed %in% seq(0,noDisast,2),]
  actData = actData[
    actData$LstratMu>=stratQts[1] & actData$LstratMu<=stratQts[2],]

  # change facet labels
  ggDataSmall$Lno_killed = paste(
    ggDataSmall$Lno_killed, 'Log(Killed)$_{r,t-1}$')
  actData$Lno_killed = paste(
    actData$Lno_killed, 'Log(Killed)$_{r,t-1}$')
...
simComboPlot=grid.arrange(
  ...
  nrow=length(stratMuIntMods))
ggsave(simComboPlot, file=paste0(
  '~/Desktop/simComboPlot_rev.pdf'), width=8, height=8)
#########################################################