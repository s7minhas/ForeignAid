...
genModelForm = function(var){
	formula(
		paste0( 
			...
			'+ (1|id)') # dyad random
			# '+ (1|id) + (1|year)') # dyad random effect		
		)
}
...
runModelParallel = function(cores=6, dataList, trainLogic, trainEnd=2002, modType, zeroInfLogic, keyRegVar, modForm, modName){
	...
	mods = foreach(ii=1:length(dataList), .packages=c('glmmADMB')) %dopar% {
		...
		m=glmmadmb(modForm, data=regData, zeroInflation=zeroInfLogic, family=modType,extra.args="-ndi 100000") # Run ,extra.args="-ndi 100000"
	} ; stopCluster(cl)
...
runModelParallel(dataList=iData, trainLogic=FALSE, modType = 'gaussian', zeroInfLogic=TRUE, keyRegVar='LstratMu')
