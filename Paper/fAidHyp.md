...
* R6/D7: What about diplomatic explanations? There are two primary explanations for the distribution of foreign aid. One view espouses the idea that allocation results from humanitarian needs and the other that it is a result of the foreign policy interests of the donor [(McKinlay & Little 1977)](http://journals.cambridge.org/action/displayAbstract?fromPage=online&aid=7661300). We should develop some measures that get at this second explanation, below are some ideas I found in the literature -- none very good. 
...
### One thing to note about network characteristics is that by using the Hoff & Westveld model, the network level variation will all be projected into the latent space. If we were using a random graph model approach, then we would be able to specify specific network level characteristics. 
