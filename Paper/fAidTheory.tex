...
\indent\indent   The main advantage of calculating the latent space of different dyadic variables as opposed to using alternative specifications such as the S Score algorithm\footnote{\citet{leeds:2007} for example creates a measure of a states ``threat environment'' as the set of all states for which ones is contiguous with or which is a major power and with an S score below the population median. } is that we are consequently able to account for third order dependencies within the data. To review, first order dependency refers the propensity for some actors to send or receive more ties than others, second-order dependency refers to reciprocity of exchange between actors while a third-order dependency refers to interactions among three or more actors. Dyadic data are rife with these types of dependencies, and aside from first-order dependency, they pose serious challenges the basic assumption of independence between observations. \\
...
\indent\indent  A visualization of the resultant dyadic PCA is shown below in Figures \ref{fig:polPCA} for the political strategic measure and in Figure \ref{fig:milPCA} for the military strategic measure for the year 2005. Along the x and y axes are the countries included in our analysis; any point within the plot represents the dyadic relationship between a country on the x and y axis with darker shading representing a stronger relationship and lighter shading representing a weaker relationship. \\
